...
public class FormClubController {
    ...
    public void ajouterClub() {
        ...
        if (!checkPresidentExists(presidentId)) {
            showAlert(Alert.AlertType.ERROR, "Erreur", "L'ID du président (" + presidentId + ") n'existe pas dans la table user.");
            return;
        }
    ...
    private boolean checkPresidentExists(int presidentId) {
        ...
        try (PreparedStatement stmt = cnx.prepareStatement(query)) {
            ...
            e.printStackTrace();
            showAlert(Alert.AlertType.ERROR, "Erreur", "Erreur lors de la vérification de l'ID du président: " + e.getMessage());
        }
        return false;
    }
